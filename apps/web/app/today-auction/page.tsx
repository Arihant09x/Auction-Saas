...
// ─── Gradient underline — fades on both sides ────────────────────────────────
...
// ─── Real Card component with GSAP hover ─────────────────────────────────────────
function AuctionCard({ auction }: { auction: any }) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="p-4 flex-1 flex flex-col bg-[#fbf9f5]">
        {/* Title & Organizer */}
        <div className="mb-2">
        ...
        {/* Date & Time
        <div className="flex items-center justify-start gap-2 text-xs text-gray-500 mb-3">
          <Calendar size={12} className="text-[#00379d] mb-1" />
        ...
        <div className="grid grid-cols-2 gap-y- gap-x-3 mt-1">
          ...
          <div className="flex flex-row items-center justify-end gap-1 text-[14px] text-gray-400">
            <span className="text-[11px] font-semibold text-[#b38200] mt-0.5">Points</span>
          ...
          <div className="flex flex-row items-center justify-start text-center gap-1 rounded-lg py-1.5 ">
            <Users size={14} className="text-[#00379d]" />
            <span className="inline-block transition-transform duration-200 hover:translate-x-[6px]">
              <span className="text-xs font-semibold text-gray-700">{auction.minPlayersPerTeam || 0} Players</span>
              <span className="text-[11px] font-semibold text-gray-400">/Team</span>
            </span>
          </div>
          <div className="flex flex-row items-center justify-end gap-1 rounded-lg py-1.5 ">
            <Clock size={14} className="text-[#00379d] mb-0.5 transition-transform duration-200 hover:translate-x-[-6px]" />
            <span className="inline-block transition-transform duration-200 hover:translate-x-[6px]">
              <span className="text-xs font-semibold text-gray-700">{displayTime || "TBD"}</span>
              <span className="text-[10px] font-semibold text-gray-400 ml-1">Time</span>
            </span>
          </div>
        </div>
      </div>
...
// =============================================================================
// ✅ FIXED SKELETON - perfect match to real card (no external dependency)
// =============================================================================
...
// ─── Grid with GSAP stagger entrance and fixed skeleton ────────────────────────
  ...
  if (!data?.data || data.data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 className="text-xl font-bold text-[#012972] mb-2">No Auctions Today</h3>
        <p className="text-gray-500 max-w-md">There are no auctions scheduled for today. Check back tomorrow or view the upcoming auctions!</p>
      </div>
    );
  }
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function TodayAuctionPage() {
  const [page, setPage] = useState(1);
  const limit = 9;

  const { data, isLoading, error } = useQuery({
    queryKey: ["today-auctions", page],
    queryFn: async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/auction/today?page=${page}&limit=${limit}`);
  ...
  return (
    <main className="relative bg-white min-h-screen font-sans overflow-x-hidden">
      {/* Fixed diagonal bands (unchanged) */}
      ...
      <div className="relative z-10 pt-[79px] px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto min-h-[70vh] flex flex-col">
          <PageTitle title="Today's" highlight="Auctions" />
